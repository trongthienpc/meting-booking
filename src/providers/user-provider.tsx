...
"use client";
import { getCurrentUser, getUserInfo, getUsersByDepartmentId } from "@/app/actions/user-actions";
...
export function UserProvider({ children }: { children: React.ReactNode }) {
  ...
  React.useEffect(() => {
    const fetchUser = async () => {
      try {
        const [result, userResponse] = await Promise.all([getUserInfo(), getCurrentUser()]);
  ...
  const getDepartmentUsers = async (departmentId: string) => {
    ...
    try {
      ...
      if (userResponse.success && userResponse?.user) {
        const user = userResponse?.user;
        setDepartmentId(user?.departmentId ?? undefined);
        const response = await getUsersByDepartmentId(departmentId, branchId);
  ...
  const value: UserContextType = { user, error, departmentUsers, isLoading, getDepartmentUsers, departmentId };
...
export function useDepartmentUsers(departmentId?: string) {
  const { departmentUsers, getDepartmentUsers, departmentId: _departmentId, isLoading, error } = useUser();
